...
FakeExtVM::FakeExtVM(EnvInfo const& _envInfo, unsigned _depth, eth::EVMSchedule _schedule):			/// TODO: XXX: remove the default argument & fix.
	ExtVMFace(_envInfo, Address(), Address(), Address(), 0, 1, bytesConstRef(), bytes(), EmptySHA3, false, _depth), schedule(_schedule)
{}
...
namespace {
 ...
 eth::EVMSchedule getEVMSchedule(eth::Network _network) {
	switch (_network) {
		case eth::Network::FrontierTest:
			return eth::FrontierSchedule;
		case eth::Network::HomesteadTest:
			return eth::HomesteadSchedule;
		case eth::Network::EIP150Test:
			return eth::EIP150Schedule;
		case eth::Network::EIP158Test:
			return eth::EIP158Schedule;
		case eth::Network::ByzantiumTest:
			return eth::ByzantiumSchedule;
		case eth::Network::ConstantinopleTest:
			return eth::ConstantinopleSchedule;
		default:
			return eth::DefaultSchedule;
	}
}
...
for (auto& i: _input.get_obj())
{
	...
	if (!TestOutputHelper::get().checkTest(testname))
		...
		for (auto const network: test::getNetworks()) {
			...
			FakeExtVM fev(env, 0, getEVMSchedule(network));
			...
			if (_fillin)
			{
				if (vmExceptionOccured)
				{
					...
					BOOST_REQUIRE_MESSAGE(networkTestOutput.count("expect") == 0, testname + " expect should have been erased!");
				}
				else
				{
					networkTestOutput["post"] = mValue(fev.exportState());
					BOOST_REQUIRE_MESSAGE(networkTestOutput.at("post").type() == obj_type, testname + "fev.exportState returned something not an object.");
					...
					if (testInput.count("expect") > 0)
					{
						...
						State expectState(State::Null);
						AccountMaskMap expectStateMap;
						ImportTest::importState(networkTestOutput.at("post").get_obj(), postState);
						ImportTest::importState(testInput.at("expect").get_obj(), expectState, expectStateMap);
						ImportTest::compareStates(expectState, postState, expectStateMap, WhenError::Throw);
					}

					BOOST_REQUIRE_MESSAGE(networkTestOutput.count("expect") == 0, testname + " expect should never been added to the output!");

					networkTestOutput["callcreates"] = fev.exportCallCreates();
					networkTestOutput["out"] = output.size() > 4096 ? "#" + toString(output.size()) : toHexPrefixed(output);
					...
					if (testInput.count("expectOut") > 0)
					{
						...
						std::string warning = " Check State: Error! Unexpected output: " + networkTestOutput["out"].get_str() + " Expected: " + testInput.at("expectOut").get_str();
						BOOST_CHECK_MESSAGE(networkTestOutput["out"].get_str() == testInput.at("expectOut").get_str(), warning);
					}

					networkTestOutput["gas"] = toCompactHexPrefixed(fev.gas, 1);
					networkTestOutput["logs"] = exportLog(fev.sub.logs);
				}
			}
			else
			{
				string networkName = test::netIdToString(network);
				auto& testInput2 = testInput.at(networkName).get_obj();
				if (testInput2.count("post") > 0)	// No exceptions expected
				{
					BOOST_REQUIRE_MESSAGE(testInput2.at("post").type() == obj_type, testname + " post field is not an object.");
					BOOST_CHECK(!vmExceptionOccured);

					BOOST_REQUIRE_MESSAGE(testInput2.count("callcreates") > 0, testname + " callcreates field is missing.");
					BOOST_REQUIRE_MESSAGE(testInput2.at("callcreates").type() == array_type, testname + " callcreates field is not an array.");
					BOOST_REQUIRE_MESSAGE(testInput2.count("out") > 0, testname + " out field is missing.");
					BOOST_REQUIRE_MESSAGE(testInput2.count("gas") > 0, testname + " gas field is missing.");
					BOOST_REQUIRE_MESSAGE(testInput2.count("logs") > 0, testname + " logs field is missing.");
					BOOST_REQUIRE_MESSAGE(testInput2.at("logs").type() == str_type, testname + " logs field is not a string.");

					dev::test::FakeExtVM test(eth::EnvInfo{BlockHeader{}, lastBlockHashes, 0});
					test.importState(testInput2.at("post").get_obj());
					test.importCallCreates(testInput2.at("callcreates").get_array());

					checkOutput(output, testInput2);

					BOOST_CHECK_EQUAL(toInt(testInput2.at("gas")), fev.gas);

					State postState(State::Null);
					State expectState(State::Null);
					mObject mPostState = fev.exportState();
					ImportTest::importState(mPostState, postState);
					ImportTest::importState(testInput2.at("post").get_obj(), expectState);
					...
					BOOST_CHECK_EQUAL(exportLog(fev.sub.logs), testInput2.at("logs").get_str());
				}
			...
			if (_fillin)
				testOutput[test::netIdToString(network)] = networkTestOutput;
		}
	}
